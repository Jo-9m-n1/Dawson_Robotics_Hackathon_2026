...
class ultrasonic():
    # 30 ms is well past the HC-SR04's ~25 ms / ~4 m max round-trip; anything
    # that takes longer is a missed echo and we treat it as "out of range".
    _ECHO_TIMEOUT_US = 30000

    def __init__(self):
        self.Trig = Pin(0, Pin.OUT, value=0)
        self.Echo = Pin(1, Pin.IN)

    def Distance(self):
        """
        Single HC-SR04 ping. Returns distance in cm, or -1 on timeout
        (no echo within _ECHO_TIMEOUT_US). NEVER blocks indefinitely.

        The original Yahboom implementation busy-waited on Echo with no
        timeout, which hung the entire program any time the sensor failed
        to receive a return echo (target out of range, soft target, brief
        power dip during a motor pulse, etc.). This version uses
        machine.time_pulse_us, which has a hard timeout and returns
        a negative value instead of spinning forever.
        """
        self.Trig.value(0)
        time.sleep_us(2)
        self.Trig.value(1)
        ...
        try:
            duration_us = machine.time_pulse_us(
                self.Echo, 1, self._ECHO_TIMEOUT_US
            )
        except OSError:
            return -1

        if duration_us < 0:
            # -1 = pin never went high (no echo); -2 = pulse longer than timeout.
            return -1

        # HC-SR04 conversion: cm = us / 58
        return duration_us / 58.0

    def Distance_accurate(self):
        """
        Median-of-5 ping for stability. Returns int cm in [1, 500],
        or 999 on persistent timeout / out-of-range / read failure.
        """
        samples = []
        for _ in range(5):
            d = self.Distance()
        ...
        samples.sort()
        # drop one extreme on each end if we got all 5, else take the middle
        if len(samples) == 5:
            mid = (samples[1] + samples[2] + samples[3]) / 3.0
        else:
